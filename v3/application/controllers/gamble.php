...
foreach( $callers as $call ) {
 ...
 glogtext($trace);
  ...
  glogtext(  "terminate_missing_variables". "$errstr in $errfile line $errline");
...
class Gamble extends Base
{
    ...
    public function showlog() {
        ...
        $jstr = '<script src="http://s1.golf-brother.com/data/attach/js/jquery.js" type="text/javascript" charset="utf-8"></script>';
        $jstr .= '<script src="http://s1.golf-brother.com/data/attach/js/testcase.js" type="text/javascript" charset="utf-8"></script>';
        $css     = "http://s1.golf-brother.com/data/attach/css/log.css";
        ...
        echo "<div>";
        echo "<input onclick=hide_class('css_sql') type=button value=Hide_sql name=Hide_sql>";
        echo "<input onclick=hide_class('css_input') type=button value=Hide_Input name=Hide_Input>";
        echo "<input onclick=clear_gamble_log() type=button value=clear_gamble_log name=Hide_Input>";
        echo "</div>";
        $string = read_file('/var/log/php-fpm/www-error.log');
        $string = str_replace(array("\r\n", "\r", "\n"), "<br/>", $string);
        echo $string;
        echo "<br/>";
      
        $logname='gamble_log_'.date('Ymd',time());
        $date_based_file="/opt/space/webroot/ugf/log/".$logname;
       
        $filesize=filesize($date_based_file);
        $bytes = number_format($filesize / 1048576, 2) ;
        $filesize_readable=$bytes;
        echo $date_based_file;
        echo '<h1>'.$date_based_file.' [size:'.$filesize_readable.'M]</h1>';

        if($filesize_readable > 8){
            $string= "<h1>Log file size too big.</h2>";

        }
        else
        {
         $string = read_file($date_based_file);
        }
         
        $string = read_file($date_based_file);
        //$section = file_get_contents($date_based_file, NULL, NULL, 0, 14000000);
        echo $string;
        echo "</html>";
    }
     ...
     public function cleargamblelog() {
         $logname='gamble_log_'.date('Ymd',time());
         $date_based_file="/opt/space/webroot/ugf/log/".$logname;
         file_put_contents($date_based_file,"");
         file_put_contents('/var/log/php-fpm/www-error.log',"");
     }
...
//V14 添加游戏
    public function addgamble()
    {
        ...
        if($userid > 0){ 
            glogtext("beforeer_jurisdiction ");
            $user_jurisdictions = $mgr_user->get_user_jurisdiction($userid);
            glogtext("after_jurisdiction ");
        ...
        public function addgamble_fix()
    {
        ...
        if(  ! array_key_exists('rulesource', $para) ){
                if( array_key_exists('sysruleid', $gbcfg)  ){
                     $para['ruleid']=$gbcfg['sysruleid'];
                     $para['rulesource']='sys';
                } 

                
                if( array_key_exists('userruleid', $gbcfg)  ){
                     $para['ruleid']=$gbcfg['userruleid'];
                     $para['rulesource']='user';
                } 
        }
     ...
     public function batchadd(){
        ...
        foreach ($tmp_arr as $key => $one_gamble) {
    ...
    public function getBatchGame($one_batchid){
             ...
             if( count($batchgames)>0 ){
              ...
              return array('gambles'=>$batchgames,'batchid'=>$one_batchid,
                'baseunit'=>1,'baserule'=>$tmp['baserule']
                );
    }
