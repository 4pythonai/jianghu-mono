...
class MTableGridCfgExecutor extends CI_Model implements StageInterface {
    ...
    public function getter() {
        ...
        $condition = [
            [
                'rootField' => 'datagrid_type',
                'condition' => [
                    [
                        'rootValue' => 'table',
                        'displayFields' => ['base_table', 'geturl', 'delurl', 'addurl', 'updateurl']
                    ],
                    [
                        'rootValue' => 'service',
                        'displayFields' => ['service_url', 'service_para', 'service_fields']
                    ],
                    [
                        'rootValue' => 'sql',
                        'displayFields' => ['sql']
        ...
        $this->payload['ConditionCfg'] = $condition;
        return  $this->payload;
    }
    ...
    public function  setTotalColsCfg() {
        ...
        $total_cols_cfg = $this->MFieldcfg->getAllColsCfg($datagrid_code, $base_table, $all_db_fields);
        $this->payload['total_cols_cfg']  = $total_cols_cfg;
    }
    ...
    public function setTableColumnRender() {
        ...
        foreach ($this->payload['total_cols_cfg']  as $col) {
            if (!in_array($col['field_e'], array_column($this->payload['gridHiddenColumns'], 'field'))) {
                $tmp = [];
                $tmp['key'] = $col['field_e'];
                $tmp['color'] = $col['display_cfg']['column_color'];
    ...
    public function removeForbidButtons() {
        $forbidden_btn_codes = $this->db->query("select button_code from boss_act_btn_forbidden where datagridcode='{$this->payload['DataGridCode']}' and role_code='{$this->payload['role']}' ")->result_array();
        $forbidden_btn_codes = array_column($forbidden_btn_codes, 'button_code');
        $filtered = [];
        foreach ($this->payload['buttons'] as $btn) {
            if (!in_array($btn['button_code'], $forbidden_btn_codes)) {
                $filtered[] = $btn;
            }
        }
        $this->payload['buttons'] = $filtered;
