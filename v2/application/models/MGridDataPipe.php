...
class MGridDataPipe extends CI_Model implements StageInterface {
    ...
    public function init($config) {
        ...
        if ($config['id_order']) {
            $this->payload['id_order'] = $config['id_order'];
        } else {
            // 排序    缺省为倒序
    ...
    public function ifAuthorOnly() {
        ...
        // if ($this->payload['base_table'] == 'bill_tier2') {
        //     return false;
        // }
    ...
    public function setWhereString() {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $arg      = $lines[$i]['vset'];
            $pair = $this->getQueryPair($field_in_line, $arg, $operator);
    ...
    public function getQueryPair($field_in_line, $arg, $operator) {
    ...
    public function setSqlWithAuthor() {
        ...
        $sql = $this->add_sql_author($this->payload['sql_with_querycfg'], $this->payload['base_table'],  $this->payload['role'], $this->payload['user']);
    ...
    // 根据作者字段筛选数据.
    public function add_sql_author($sql, $table, $role, $currentUser) {
        if ($role == 'admin' ||  $role == 'super') {
            return $sql;
        }
        // 应该添加配置 ( datagridcode+ role 免 author检查)
        if ($role == 'finance') {
            return $sql;
        }
    ...
    public function setAllRows() {
        ...
        $replacement = 'SELECT COUNT($1) AS total FROM';

        $modified_sql = preg_replace($pattern, $replacement,  $this->payload['sql_quick']);

        $row = $this->db->query($modified_sql)->row_array();
        $total = $row['total'];

        // $this->payload['all_rows'] = $all_rows;
        $this->payload['total'] =  $total;
        // $this->payload['all_rows'] = [];
    }
    ...
    public function seekTransformered($table, $field) {
        ...
        // debug($table);
        // debug($field);
        // debug($this->payload['transformered_fields']);
