...
class MOrganization  extends CI_Model {
  ...
  public function getDeptMembers($deptid) {
    $sql = "select id, id as 'key', user,staff_name as username from   nanx_user     where  deptid= $deptid ";
  ...
  function getUserScope($user) {
    ...
    if ($isLeader) {
      ...
      $sql = "select user from nanx_user where deptid in( $depts_str) ";
  ...
  public function ViewRoleUser($role) {
    $sql = "select * from nanx_user_role_assign where role_code = '$role' ";
