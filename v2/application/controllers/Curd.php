...
class Curd extends MY_Controller {
    ...
    public function listData() {
        ...
        if (!array_key_exists('id_order', $para)) {
    ...
    private function GetTmpExcelData($config) {
        $currentPage = isset($config['currentPage']) ? (int)$config['currentPage'] : 1;
        $pageSize = isset($config['pageSize']) ? (int)$config['pageSize'] : 10;
        $offset = ($currentPage - 1) * $pageSize;

        // 获取数据
        $this->db->select('*');
        $this->db->from('tmp_excel');
        ...
        // 获取总记录数
        $totalQuery = $this->db->query('SELECT COUNT(*) AS total FROM tmp_excel');
    ...
    public function addData() {
        ...
        $rawData = $this->fix_ghost($rawData);
