...
class DataGrid extends MY_Controller {
  ...
  public function getGridFieldConfig() {
    ...
    $baseTable = $this->MDataGrid->getBaseTableByActcode($dataGridCode);

    $fullColumns = $this->getFullColumns($dataGridCode, $baseTable);

    $fixed = [];
    foreach ($fullColumns as $column) {
      $tmp = $this->initColumnConfig($column);

      $this->setDisplayConfig($tmp, $dataGridCode, $column['Field']);
      $this->setForbiddenAttributes($tmp, $dataGridCode, $column['Field']);
      $this->setEditorConfig($tmp, $dataGridCode, $baseTable, $column['Field']);
      $this->setCategoryConfig($tmp, $dataGridCode, $baseTable, $column['Field']);

      $fixed[] = $tmp;
    }

    $ret['code'] = 200;
    $ret['sorted'] = $this->getSortingSql($dataGridCode);
    $ret['data'] = $this->MFieldcfg->_sortFieldDisplayOrder($fixed, $this->getDisplayOrder($dataGridCode));

    echo json_encode($ret);
  }

  private function getFullColumns($dataGridCode, $baseTable) {
    if ($baseTable) {
      return $this->db->query("SHOW FULL FIELDS FROM $baseTable")->result_array();
    } else {
      $this->db->where('datagrid_code', $dataGridCode);
      $tmp = $this->db->get('nanx_activity')->row_array();
      $cols = json_decode($tmp['service_fields'], true);
      return array_map(function ($col) {
        return ['Field' => $col, 'Type' => 'string', 'Comment' => $col];
      }, $cols);
    }
  }

  private function initColumnConfig($column) {
    return [
      'Field' => $column['Field'],
      'Type' => $column['Type'],
      ...
      'form_hidden' => false,
      'column_hidden' => false,
      'plugid' => $this->MFieldcfg->toUnformType($column['Type']),
      'readonly' => false,
      'uform_para' => null,
      'default_v' => null,
      'defaultv_para' => null,
  ...
  private function setDisplayConfig(&$tmp, $dataGridCode, $field) {
    $this->db->where(['datagrid_code' => $dataGridCode, 'field_e' => $field]);
    $this->db->select(['field_c', 'field_width', 'label_width', 'show_as_pic', 'width', 'handler', 'column_color']);
    ...
    if ($row) {
      ...
      $tmp['column_color'] = $row['column_color'];
    }
  }

  private function setForbiddenAttributes(&$tmp, $dataGridCode, $field) {
    $formHiddenFields = array_column($this->MFieldcfg->getForbiddenFields($dataGridCode, 'form_hidden'), 'field');
    $tmp['form_hidden'] = in_array($field, $formHiddenFields);

    $columnHiddenFields = array_column($this->MFieldcfg->getForbiddenFields($dataGridCode, 'column_hidden'), 'field');
    $tmp['column_hidden'] = in_array($field, $columnHiddenFields);
  }

  private function setEditorConfig(&$tmp, $dataGridCode, $baseTable, $field) {
    $this->db->where(['datagrid_code' => $dataGridCode, 'base_table' => $baseTable, 'field_e' => $field]);
    $row = $this->db->get('nanx_biz_column_editor_cfg')->row_array();

    if ($row) {
      $tmp['readonly'] = intval($row['readonly']) == 1;
      $tmp['plugid'] = strlen($row['uform_plugin']) > 3 ? $row['uform_plugin'] : $tmp['plugid'];
  ...
  private function setCategoryConfig(&$tmp, $dataGridCode, $baseTable, $field) {
    $this->db->where(['actcode' => $dataGridCode, 'base_table' => $baseTable, 'field_e' => $field]);
  ...
  private function getSortingSql($dataGridCode) {
    return "SELECT DISTINCT datagrid_code, column_field FROM nanx_activity_column_order WHERE datagrid_code='$dataGridCode'";
  }

  private function getDisplayOrder($dataGridCode) {
    return $this->db->query($this->getSortingSql($dataGridCode))->result_array();
  }
  ...
  public function saveFieldCfgHandler($dataGridCode, $base_table, $para) {
    ...
    // 初颜色配置
    $this->_saveFieldColumnColorHandler($dataGridCode, $base_table, $para);
  }


  private function _saveFieldColumnColorHandler($dataGridCode, $base_table, $para) {

    // 修改  label
    if (array_key_exists('ColumnColor', $para)) {
      if (strlen($para['column_color']) > 1) {
        ...
        if (count($rows) == 1) {
          $this->db->update('nanx_activity_field_special_display_cfg', ['column_color' => $para['column_color']], $wherecfg);
        } else {
          $wherecfg['column_color'] = $para['column_color'];
