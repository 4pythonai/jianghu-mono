...
CREATE TABLE `t_user` (
  ...
  `display_name` VARCHAR(100) DEFAULT NULL COMMENT '显示名称（用户可修改）',
...
+ 非注册用户(n)没有 phone；已合并用户(m)的 phone 会被置空

---

## 二、名称字段设计

> 详细迁移指南见 [nickname-display-remark.md](./nickname-display-remark.md)
...
| `display_name` | 用户输入 | 是 | 用户可随时修改，默认值=wx_name |
| `remark_name` | 他人设置 | 是 | A给B设置的名字，仅A可见（存在 user_remark 表） |

### 显示优先级

```plain
remark_name > display_name > wx_name
```

### 各场景下的名称处理

| 场景 | wx_name | display_name |
|------|---------|--------------|
...
5. **名称处理**：`display_name = wx_name`（用微信名称覆盖）
...
**显示优先级**：`remark_name` > `display_name` > `wx_name`

**用户合并**：合并后 display_name 被 wx_name 覆盖，但他人设置的备注名保留
...
3. 如果没有 → 返回 B 的 display_name
4. 如果 display_name 为空 → 返回 B 的 wx_name
```
