...
class MTeam extends CI_Model {
     ...
     * 获取球队成员列表
     */
    public function getTeamMembers($user_id, $team_id, $status = 'active') {
        // 确保 user_id 是整数类型，防止 SQL 注入
        $user_id = (int)$user_id;
        ...
        $this->db->join('t_user_remark ur', "ur.user_id = {$user_id} AND ur.target_id = u.id", 'left', false);
