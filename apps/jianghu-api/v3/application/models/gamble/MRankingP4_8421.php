...
class MRankingP4_8421 extends CI_Model {
    ...
    public function rankAttenders($holeIndex, &$hole, $context) {
        ...
        switch ($tieResolveConfig) {
            case 'indicator.reverse_indicator':
                $this->addDebug($hole, "🔴🟢🔵  :规则1:得分相同按出身得分排序");
                $ranking = $this->rankByIndicatorReverseIndicator($holeIndex, $hole, $context, $bootStrapOrder);
                break;

            case 'indicator.win_loss.reverse_win':
                $this->addDebug($hole, "🔴🟢🔵  :规则2:得分相同按输赢排序，输赢相同按出身得分排序");
                $ranking = $this->rankByIndicatorWinLossReverseWin($holeIndex, $hole, $context, $bootStrapOrder);
                break;

            case 'indicator.win_loss.reverse_indicator':
                $this->addDebug($hole, "🔴🟢🔵  :规则3:得分相同按输赢排序，输赢相同按出身得分排序");
                $ranking = $this->rankByIndicatorWinLossReverseIndicator($holeIndex, $hole, $context, $bootStrapOrder);
                break;
    ...
    private function rankByIndicatorReverseIndicator($holeIndex, &$hole, $context, $bootStrapOrder) {
        ...
        foreach ($users as $user_id) {
            $nickname = $this->getNicknameByUserid($user_id, $context);
            $indicator = $hole['indicators_8421'][$user_id];
            $beforeSortInfo[] = "🏌️ {$nickname}(ID:{$user_id}) 得分:{$indicator}";
        }
        ...
        usort($users, function ($auser, $bUser) use (&$hole, $holeIndex, $context) {
            ...
            if ($indicatorA !== $indicatorB) {
                ...
                $this->addDebug($hole, "✅ 得分不同，{$winner} 排名更高");
                return $result;
            }
        ...
        for ($i = 0; $i < count($users); $i++) {
            $user_id = $users[$i];
            $nickname = $this->getNicknameByUserid($user_id, $context);
            $indicator = $hole['indicators_8421'][$user_id] ?? 0;
            $rank = $i + 1;
            $afterSortInfo[] = "#{$rank} {$nickname}(ID:{$user_id}) 得分:{$indicator}";
        }
    ...
    private function rankByIndicatorWinLossReverseWin($holeIndex, &$hole, $context, $bootStrapOrder) {
        ...
        foreach ($users as $user_id) {
            ...
            $beforeSortInfo[] = "🏌️ {$nickname}(ID:{$user_id}) 得分:{$indicator} {$winStatus}";
        }
        $this->addDebug($hole, "📊 排序前状态: " . implode(', ', $beforeSortInfo));
        ...
        usort($users, function ($auser, $bUser) use (&$hole, $holeIndex, $context) {
            ...
            $indicatorB = $hole['indicators_8421'][$bUser] ?? 0;
            ...
            $this->addDebug($hole, "⚖️ 得分相同({$indicatorA})，比较输赢状态");
            // 得分相同，按输赢排序
        ...
        for ($i = 0; $i < count($users); $i++) {
            ...
            $winStatus = $isWinner ? "🥇胜" : "🥈负";
            $rank = $i + 1;
            $afterSortInfo[] = "#{$rank} {$nickname}(ID:{$user_id}) 得分:{$indicator} {$winStatus}";
        }
    ...
    private function rankByIndicatorWinLossReverseIndicator($holeIndex, &$hole, $context, $bootStrapOrder) {
        ...
        usort($users, function ($auser, $bUser) use (&$hole, $holeIndex, $context) {
            ...
            $this->addDebug($hole, "🔄 输赢相同，回溯历史得分比较");
            // 输赢相同，回溯历史得分
            return $this->compareByHistoryIndicator($auser, $bUser, $holeIndex, $context);
        });
    ...
    private function rankByScoreReverseScore($holeIndex, &$hole, $context, $bootStrapOrder) {
        ...
        foreach ($users as $user_id) {
            ...
            $beforeSortInfo[] = "🏌️ {$nickname}(ID:{$user_id}) 成绩:{$score}";
        }
        $this->addDebug($hole, "📊 排序前状态: " . implode(', ', $beforeSortInfo));
    ...
    private function rankByScoreWinLossReverseWin($holeIndex, &$hole, $context, $bootStrapOrder) {
        ...
        foreach ($users as $user_id) {
            ...
            $isWinner = $this->isUserWinner($user_id, $hole);
            $winStatus = $isWinner ? "🥇胜" : "🥈负";
            $beforeSortInfo[] = "🏌️ {$nickname}(ID:{$user_id}) 成绩:{$score} {$winStatus}";
        }
        $this->addDebug($hole, "📊 排序前状态: " . implode(', ', $beforeSortInfo));
