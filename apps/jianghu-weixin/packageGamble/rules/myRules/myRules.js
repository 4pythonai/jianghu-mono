...
Page({
    ...
    onLoad(options = {}) {
        ...
        const backUrl = backQuery
            ? `/packageGame/gameDetail/gamble/gamble?${backQuery}`
            : `/packageGame/gameDetail/gamble/gamble`;
    ...
    _redirectToTab(tab, options) {
        ...
        const url = query ? `/packageGamble/rules/${tab}/${tab}?${query}` : `/packageGamble/rules/${tab}/${tab}`;
        wx.redirectTo({ url });
    },
